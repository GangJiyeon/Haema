...
// 업체 선택 Select Box
let biz_select_box = document.querySelector("#biz_select");
...
(function () {
    ...
    $(document).ready(function(){
        ...
        businessesList();
        console.log(localStorage.getItem(bizSelectValue))

        if (localStorage.getItem(bizSelectValue) != 1 ){
            //$("#biz_select").val(localStorage.getItem(bizSelectValue)).prop("selected", true); 
            const el = document.getElementById('biz_select');  //select box
            const len = el.options.length; 
            const str = localStorage.getItem(bizSelectValue); 
...
function businessesList(){
     ...
     $.ajax({
        ...
        , success : function(e){
            ...
            // isImp(노출 여부 트루인 것만 필터링)
            e.body = e.body.filter(item => item.isImp == true);
...
function onClickAddButton(arr) {
    let biz_select_box_after_onload = $("#biz_select");
    ...
    $.ajax({
        ...
        success : function (res) {
            $.each(res, (index, item) => {
                ...
                var bizStr = item.biz_code
                objOption.innerText = item.biz_name;
                objOption.text = item.biz_name;
                objOption.value = bizStr;
                biz_select_box_after_onload.append(objOption);
            });
...
function onChangeBusinessSelectBox(){
    ...
    let currentValue = $("#biz_select").val();

    if(currentValue != "1") {
        localStorage.setItem(bizSelectValue,currentValue);
    } else {
        localStorage.setItem(bizSelectValue, "1");
    }
